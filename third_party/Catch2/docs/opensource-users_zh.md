...
Catch2 非常适合开源项目。它采用 [Boost Software License (BSL)](../LICENSE.txt) 授权，没有额外依赖，并支持双文件分发。

因此，Catch2 被很多不同的开源项目用于测试。这个页面列出了一些项目，不过显然它永远不会完整（而且也没有完整的目标）。注意，下面的列表有意按字母顺序排列，以避免暗示这些项目的重要性有先后。

_请只在你是该项目维护者，或者得到维护者明确同意的情况下，把项目添加到这里。_
