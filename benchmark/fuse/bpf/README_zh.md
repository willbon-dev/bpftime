...
# 进程会保持运行并打印每个 syscall 的操作次数，停止后会自动卸载
```
...
getdents64 syscall 的基线

- 使用 Passthrough fuse 测试：
...
在控制平面中过滤某些路径，通过基于 uid 的过滤限制对某些文件的访问：
